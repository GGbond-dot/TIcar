...
static void H_K_R_Adjustment(KalmanFilter_t *kf)
{
    ...
    {
        if (kf->MeasurementMap[i])
        {
            kf->MeasurementValidNum++;
            kf->temp[kf->MeasurementValidNum - 1] = i;
            kf->MeasurementMap[i] = 0;
        }
    }
    ...
    {
        kf->MeasurementMap[kf->temp[i]] = 1;
        kf->MeasurementDegree[kf->temp[i]] = 1.0f;
    }

    for (uint8_t i = 0; i < kf->MeasurementValidNum; ++i)
    {
        kf->z_data[i] = kf->MeasuredVector[kf->temp[i]];
        kf->MatR_DiagonalElements[i] = kf->MatR_DiagonalElements[kf->temp[i]];
    }

    for (uint8_t i = 0; i < kf->MeasurementValidNum * kf->xhatSize; ++i)
        kf->H_data[i] = 0;

    for (uint8_t i = 0; i < kf->MeasurementValidNum; ++i)
        kf->H_data[i * kf->xhatSize + kf->temp[i]] = kf->MeasurementDegree[kf->temp[i]];

    for (uint8_t i = 0; i < kf->MeasurementValidNum * kf->MeasurementValidNum; ++i)
        kf->R_data[i] = 0;

    for (uint8_t i = 0; i < kf->MeasurementValidNum; ++i)
        kf->R_data[i * kf->MeasurementValidNum + i] = kf->MatR_DiagonalElements[i];
