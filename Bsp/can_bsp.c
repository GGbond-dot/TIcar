...
************************************************************************
**/
static void can_filter_configure(FDCAN_HandleTypeDef *hfdcan)
{
	...
	fdcan_filter.FilterID2 = 0x07FF;                               //如果FDCAN配置为传统模式的话，这里是32位掩码
	if(HAL_FDCAN_ConfigFilter(hfdcan,&fdcan_filter)!=HAL_OK) 		 //滤波器初始化
	{
		Error_Handler();
	}
...
{
	can_filter_configure(&hfdcan1);
	can_filter_configure(&hfdcan2);
	can_filter_configure(&hfdcan3);
}
/**
************************************************************************
...
************************************************************************
**/
static uint8_t rx_data1[8] = {0};
static uint8_t rx_data2[8] = {0};
static uint8_t rx_data3[8] = {0};

void fdcan1_rx_callback(void)
{
	FDCAN_RxHeaderTypeDef rx_header;
	if(HAL_FDCAN_GetRxMessage(&hfdcan1,FDCAN_RX_FIFO0, &rx_header, rx_data1)!=HAL_OK)
	{
		return;
	}
	CAN_UserRxCallback(&hfdcan1, &rx_header, rx_data1);
}

void fdcan2_rx_callback(void)
{
	FDCAN_RxHeaderTypeDef rx_header;
	if(HAL_FDCAN_GetRxMessage(&hfdcan2,FDCAN_RX_FIFO0, &rx_header, rx_data2)!=HAL_OK)
	{
		return;
	}
	CAN_UserRxCallback(&hfdcan2, &rx_header, rx_data2);
}

void fdcan3_rx_callback(void)
{
	FDCAN_RxHeaderTypeDef rx_header;
	if(HAL_FDCAN_GetRxMessage(&hfdcan3,FDCAN_RX_FIFO0, &rx_header, rx_data3)!=HAL_OK)
	{
		return;
	}
	CAN_UserRxCallback(&hfdcan3, &rx_header, rx_data3);
}

__weak void CAN_UserRxCallback(FDCAN_HandleTypeDef *hfdcan, const FDCAN_RxHeaderTypeDef *header, const uint8_t *data)
{
	UNUSED(hfdcan);
	UNUSED(header);
	UNUSED(data);
